...
// TaskStatus returns the task group status and latest deployment status
// for a given job.
func (n *Nomad) TaskStatus(jobID string, so *SearchOptions) ([]*models.TaskGroupStatus, string, error) {
	if so == nil {
		so = &SearchOptions{}
	}
